...
namespace Framework
{
	...
	{
		//Constructor
		ExpDate_FilterProxyModel::ExpDate_FilterProxyModel(ExpGraph* os,QObject *parent /* = 0 */):QSortFilterProxyModel(parent)
		{
			oscanner = os;
		}///End constructor

		QDate ExpDate_FilterProxyModel::ConvertToDate(const QString& dt) const
		{
			QStringList lst =  dt.split("/");//0 = Month ; 1= day ; 2= Year
			return QDate(lst[2].toInt(),lst[0].toInt(),lst[1].toInt());
		}

		bool ExpDate_FilterProxyModel::lessThan(const QModelIndex &left, const QModelIndex &right) const
		{
		...
		bool ExpDate_FilterProxyModel::filterAcceptsColumn(int source_column, const QModelIndex &source_parent) const
		{
		...
		void ExpDate_FilterProxyModel::FilterSpecificColumn(int col_no)
		{
			ColumnNoToFilter.push_back(col_no);
		}

		//Display this column
		void ExpDate_FilterProxyModel::UndoFilterSpecificColumn(int col_no)
		{
		...
		//-------------------------------------------------------------------------------------------
		void ExpDate_FilterProxyModel::SetFilterExpressionFromLayout(std::map<int,std::string> layoutMap)
		{
			m_columnPatterns = layoutMap;
		}

		std::map<int,std::string>  ExpDate_FilterProxyModel::GetFilterExpressionCollection()
		{
			return m_columnPatterns;
		}

		void ExpDate_FilterProxyModel::SetColumnsToHideFromLayout(std::vector<int> layoutMap)
		{
			ColumnNoToFilter = layoutMap;
		}
		std::vector<int>  ExpDate_FilterProxyModel::GetHideCoulmnCollection()
		{
		...
		void ExpDate_FilterProxyModel::AddFilterToColumn(int col_no,std::string filter_string,QStandardItemModel* frmPtrModel)
		{
			m_columnPatterns[col_no] = filter_string;
		}

		void ExpDate_FilterProxyModel::RemoveFilterFromColumn(int col_no,QStandardItemModel* frmPtrModel)
		{
			m_columnPatterns.erase(col_no);
		}


		bool ExpDate_FilterProxyModel::ShouldRowBeDisplayed(const std::string filter_string,const std::string rx_val) const
		{
			using namespace qi;
		...
		bool ExpDate_FilterProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
		{
		...
		float ExpDate_FilterProxyModel::trunc(float d) const
		{
			return (d>0) ? floor(d) : ceil(d) ; 
		}

		//Yes they are equal
		bool ExpDate_FilterProxyModel::FloatCompareOptions(float a , float b) const
		{
