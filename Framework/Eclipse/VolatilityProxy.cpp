...
namespace Framework
{
	...
	{
		//Constructor
		VolatilityGraph_FilterProxyModel::VolatilityGraph_FilterProxyModel(VolatilityGraph* os,QObject *parent /* = 0 */):QSortFilterProxyModel(parent)
		{
			volatplot = os;
		}///End constructor

		QDate VolatilityGraph_FilterProxyModel::ConvertToDate(const QString& dt) const
		{
			QStringList lst =  dt.split("/");//0 = Month ; 1= day ; 2= Year
			return QDate(lst[2].toInt(),lst[0].toInt(),lst[1].toInt());
		}

		bool VolatilityGraph_FilterProxyModel::lessThan(const QModelIndex &left, const QModelIndex &right) const
		{
		...
		bool VolatilityGraph_FilterProxyModel::filterAcceptsColumn(int source_column, const QModelIndex &source_parent) const
		{
		...
		void VolatilityGraph_FilterProxyModel::FilterSpecificColumn(int col_no)
		{
			ColumnNoToFilter.push_back(col_no);
		}

		//Display this column
		void VolatilityGraph_FilterProxyModel::UndoFilterSpecificColumn(int col_no)
		{
		...
		//-------------------------------------------------------------------------------------------
		void VolatilityGraph_FilterProxyModel::SetFilterExpressionFromLayout(std::map<int,std::string> layoutMap)
		{
			m_columnPatterns = layoutMap;
		}

		std::map<int,std::string>  VolatilityGraph_FilterProxyModel::GetFilterExpressionCollection()
		{
			return m_columnPatterns;
		}

		void VolatilityGraph_FilterProxyModel::SetColumnsToHideFromLayout(std::vector<int> layoutMap)
		{
			ColumnNoToFilter = layoutMap;
		}
		std::vector<int>  VolatilityGraph_FilterProxyModel::GetHideCoulmnCollection()
		{
		...
		void VolatilityGraph_FilterProxyModel::AddFilterToColumn(int col_no,std::string filter_string,QStandardItemModel* frmPtrModel)
		{
			m_columnPatterns[col_no] = filter_string;
		}

		void VolatilityGraph_FilterProxyModel::RemoveFilterFromColumn(int col_no,QStandardItemModel* frmPtrModel)
		{
			m_columnPatterns.erase(col_no);
		}


		bool VolatilityGraph_FilterProxyModel::ShouldRowBeDisplayed(const std::string filter_string,const std::string rx_val) const
		{
			using namespace qi;
		...
		bool VolatilityGraph_FilterProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
		{
		...
		float VolatilityGraph_FilterProxyModel::trunc(float d) const
		{
			return (d>0) ? floor(d) : ceil(d) ; 
		}

		//Yes they are equal
		bool VolatilityGraph_FilterProxyModel::FloatCompareOptions(float a , float b) const
		{
